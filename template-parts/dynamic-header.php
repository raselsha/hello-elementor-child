	...
	<div class="container site-header-nav">
		<div class="site-branding show-<?php echo esc_attr( hello_elementor_get_setting( 'hello_header_logo_type' ) ); ?>">
			<?php if ( has_custom_logo() && ( 'title' !== hello_elementor_get_setting( 'hello_header_logo_type' ) || $is_editor ) ) : ?>
				<div id="logo" class="image-logo site-logo <?php echo esc_attr( hello_show_or_hide( 'hello_header_logo_display' ) ); ?>">
					<?php if ( has_custom_logo() ) { ?>
							<?php 
								$custom_logo_id = get_theme_mod( 'custom_logo' );
								$image = wp_get_attachment_image_src( $custom_logo_id , 'full' );
							?>
							<a href="<?php echo site_url(); ?>"><img class="logo-img" src="<?php echo $image[0]; ?>" ></a>
					<?php } ?>
				</div>
			<?php 
				elseif ( $site_name && ( 'logo' !== hello_elementor_get_setting( 'hello_header_logo_type' ) || $is_editor ) ) : ?>
		...
		<?php if ( $header_nav_menu ) : ?>
			<nav id="navigation" class="site-navigation <?php echo esc_attr( hello_show_or_hide( 'hello_header_menu_display' ) ); ?>">
				<?php
				// PHPCS - escaped by WordPress with "wp_nav_menu"
				echo $header_nav_menu; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				?>
				<a href="#" class="call-to-action">Get in touch</a>
			</nav>
